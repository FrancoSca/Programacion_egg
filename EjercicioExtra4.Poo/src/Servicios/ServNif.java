...
public class ServNif {
    ...
    public Nif crearNif (Nif nif){
        ...
        nif.setRestDni( (int) (nif.getDni() % 23));
        for (int i = 0; i < 22; i++) {
            if (nif.getRestDni() == i) {
                nif.setNif(nif.getLetra()[i]);
            }
        }
        return nif;
    }
    public void mostrarNif (Nif nif){
        String Nif = String.valueOf(nif.getDni());
        System.out.println("El numero de documento con el nif es: " + Nif + "-" + nif.getNif());
    }
